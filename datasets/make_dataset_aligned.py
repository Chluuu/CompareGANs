...
def align_images(a_file_paths, b_file_paths, target_path):
    ...
    for i in range(len(a_file_paths)):
        ...
        aligned_image = Image.new("L", (img_a.size[0] * 2, img_a.size[1]))
        aligned_image.paste(img_a, (0, 0))
        aligned_image.paste(img_b, (img_a.size[0], 0))
        aligned_image.save(os.path.join(target_path, '{:04d}.png'.format(i))) #changed 04/20

        '''L=Image.open(os.path.join(target_path, '{:04d}.png'.format(i))).convert('L')
        R=transforms.Resize(imsize)(L)
        R.save(os.path.join(target_path, '{:04d}.png'.format(i)))'''
